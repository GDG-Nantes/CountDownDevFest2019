...
class ParserMidi {
  ...
  initListeners() {
    this.midiPlayer.on("fileLoaded", () => {
      ...
      this.midiPlayer.events.forEach(trackEvent => {
        ...
        if (
          firstEventTrack.name === "Sequence/Track Name" &&
  ...
  initListenersTone(){
    Midi.fromUrl(`./assets/songs/${this.fileToParse}/notes.mid`).then(midi => {
        midi.tracks.forEach(track => {
          if (track.name === "PART GUITAR") {
            track.notes.forEach(note => this.processMidiToneEvent(note));
          }
        });
        this.finalizeSong();
      });
  ...
  processMidiToneEvent(note) {
    let tickEvent = this.objectSong.tickMap[note.ticks];
    if (!tickEvent) {
      tickEvent = {
        tick: note.time,
        tracks: [],
      ...
      this.objectSong.tickMap[note.ticks] = tickEvent;
      this.objectSong.tickArray.push(tickEvent);
    }
    if (event.midi >= 60 && event.midi < 70) {
      tickEvent.tracks.push(`1`);
    } else if (event.midi >= 70 && event.midi < 80) {
      tickEvent.tracks.push(`2`);
    } else if (event.midi >= 80 && event.midi < 90) {
      tickEvent.tracks.push(`3`);
    } else if (event.midi >= 90 && event.midi < 100) {
      tickEvent.tracks.push(`4`);
    } else if (event.midi >= 100 && event.midi < 110) {
  ...
  finalizeSong() {
    this.objectSongTone.tickArray = this.objectSongTone.tickArray.sort(
      (tickA, tickB) => tickA.tick - tickB.tick
    );
    let data = JSON.stringify(this.objectSongTone, null, 2);
